...
jl::Client::Client(std::string name, std::string password) : User(name, password){}

void jl::Client::setName(std::string name)
{
    m_name = name;
}

void jl::Client::setPassword(std::string password){
    if(password.empty()){
        m_password="UNKNOWN";           
    }else{
        m_password=password;
...
std::string jl::Client::getName()const{
    return m_name;
}

    
std::string jl::Client::getPassword()const{
    return m_password;
}
   
  
void jl::Client::login (bool &p2){
    bool found=false;
    std::string l, line;
...
void jl::Client::loadS(std::vector<std::vector<int>>&matrix,std::string f){
...
void jl::Client::pickSeat(std::vector<std::vector<int>>matrix,std::string f,int &row,int &column){
    cout<<"Choose row: ";
    cin>>row;
    cout<<"Choose column:" ;
    cin>>column;
...
void jl::Client::buyTicket(std::vector<Movie>movies,int &row,int &column,std::vector<std::vector<int>>&matrix1,std::vector<std::vector<int>>&matrix2){
    loadM(movies);
    showMovies();
    int pick;
