...
jl::Admin::Admin(std::string name,std::string password) : User(name,password){};
   
void jl::Admin::setName(std::string name){
    if(name.empty()){                       
        m_name="UNKNOWN";       
    }else{      
        m_name=name;     
...
void jl::Admin::setPassword(std::string password){ 
    if(password.empty()){ 
        m_password="UNKNOWN";  
    }else{   
        m_password=password;  
...
std::string jl::Admin::getName()const{
    return m_name;
}

std::string jl::Admin::getPassword()const{
    return m_password;
}

  
void jl::Admin::login(bool &p2){
    bool found=false;
    std::string l, line;
...
void jl::Admin::loadS(std::vector<std::vector<int>>&matrix,std::string f){
    std::ifstream file(f);
    if (!file.is_open()) {
        cout << "Error" <<endl;
        return;
...
void jl::Admin::addMovie(std::vector<Movie>movies){
...
void jl::Admin::reset(std::vector<std::vector<int>>&matrix,std::string f){
    for(int i=0;i<5;i++){
    for(int j=0;j<5;j++){
         matrix[i][j]=1;
    }
